...
//        long normalizedUtcStartDay = SunshineDateUtils.normalizeDate(now);
        ...
        for (int i = 0; i < jsonWeatherArray.length(); i++) {
            ...
            pressure = mainObject.getDouble(OWM_PRESSURE);  // needs to be in main
            humidity = mainObject.getInt(OWM_HUMIDITY);  // needs to be in main

            JSONObject windObject = dayForecast.getJSONObject(OWM_WIND);
            windSpeed = windObject.getDouble(OWM_WINDSPEED); //good
            windDirection = windObject.getDouble(OWM_WIND_DIRECTION); //good
            ...
            JSONObject weatherObject =
                    dayForecast.getJSONArray(OWM_WEATHER).getJSONObject(0); //good

            weatherId = weatherObject.getInt(OWM_WEATHER_ID); //good
